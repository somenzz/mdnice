...
function addSpaces(text) {
  ...
  const re2 = /([a-zA-Z0-9]+)([\p{Script=Han}]+)/gu;
  const re3 = /([\x21-\x2F\x3A-\x40\x5B-\x60\x7B-\x7E]+)([\p{Script=Han}]+)/gu; // ASCII punctuation
  const re4 = /([\p{Script=Han}]+)([\x21-\x2F\x3A-\x40\x5B-\x60\x7B-\x7E]+)/gu; // ASCII punctuation

  // Apply regular expressions.
  text = text.replace(re1, "$1 $2");
  text = text.replace(re2, "$1 $2");
  text = text.replace(re3, "$1 $2");
  text = text.replace(re4, "$1 $2");
